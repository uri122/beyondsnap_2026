...
const PACKAGES = [
  ...
  },
  {
    ...
    items: [
      "1인 촬영",
      "14inch x 11inch 50p 1권 + 10inch x 8inch 50p 2권",
      "원본 + 편집본 링크로 전송",
    ],
    hasImage: true,
  },
          ...
          >
            ...
            <div className={`md:w-58 ${idx === 0 && "flex items-center"}`}>
              {idx === 0 ? (
                ...
                <>
                  <h2 className="mt-2 font-serif text-4xl text-neutral-700 before:content-[''] before:border-l-5 before:border-accent-rose before:pl-4">
                    {pkg.name}
                  </h2>
            ...
            <div>
              ...
              <ul
                className={`${idx === 0 ? "text-xs space-y-1" : "text-base space-y-2"} leading-relaxed text-neutral-900`}
              >
                {pkg.items.map((item) => (
                  <li key={item} className="flex gap-2">
                    {idx !== 0 && (
                      <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-accent-rose" />
                    )}
                    <span>{item}</span>
                  </li>
                ))}
                  ...
                  <dl className="mt-4 divide-y divide-accent-rose-line">
                    {MAIN_SNAP_ADDONS.map((addon) => (
                      ...
                      >
                        ...
                        <dd className="shrink-0 text-neutral-500">
                          {addon.price}
                        </dd>
                      </div>
                    ))}
